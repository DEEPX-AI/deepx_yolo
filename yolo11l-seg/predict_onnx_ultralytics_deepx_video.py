"""
YOLOv11 Segmentation ONNX Inference using Custom Ultralytics DEEPX Library
...
1. INFERENCE TIME (Pure Inference Only)
   ...
   └─ [MEASURED] Postprocessing (NMS, mask generation, coordinate scaling, Results creation)
...
def run_video_inference(model_path, video_path, output_dir, debug=False, save=True, show=False):
    ...
    Timing Measurements:
        ...
        inference_time (Pure Inference):
            ...
            - Includes ONLY: preprocessing + inference + postprocessing (NMS, mask generation)
    ...
    try:
        ...
        # Load the DXNN model (use task='segment' for semantic segmentation)
        ...
        video_writer = None
        if save:
            video_filename = Path(video_path).stem + f'_segmented_{timestamp}.mp4'
...
def main():
    ...
    if video_statistics:
        ...
        print(f"  │  └─ Postprocessing:    (NMS, mask generation, coordinate scaling)")
        print(f"  │")
